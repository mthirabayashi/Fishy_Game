	    ...
	    value: function updateScore() {
	      if (this.gameOver === false) {
	        ...
	        if (this.score < 10) {
	          ...
	          var seconds = this.score - minutes * 60;
	        }
	      ...
	      function handleTick(e) {
	        ...
	        this.fish.forEach(function (el, idx) {
	          ...
	          if (Util.wrap(el, stage)) {
	            ...
	            newFish = [].concat(_this.fish.splice(0, idx).concat(_this.fish.splice(1)));
	            _this.fish = newFish;
	          }
	          if (Util.collision(el, _this.hero[0])) {
	            console.log('collision!');
	            if (el.fish.width > _this.hero[0].hero.width) {
	              createjs.Ticker.reset();
	              var lose = new createjs.Text("Game Over", "40px Arial", "#05B");
	              lose.x = 280;
	              lose.y = 200;
	              stage.addChild(lose);
	              stage.removeChild(_this.hero[0].hero);
	              stage.update();
	              $('#replay').removeClass('hidden');
	              _this.gameOver = true;
	              return;
	            }
	            if (_this.hero[0].hero.width > stage.canvas.width) {
	              createjs.Ticker.reset();
	              ...
	              _this.fish.forEach(function (el) {
	                stage.removeChild(el.fish);
	              });
	              stage.update();
	              $('#replay').removeClass('hidden');
	            ...
	            newFish = [].concat(_this.fish.splice(0, idx).concat(_this.fish.splice(1)));
	            _this.fish = newFish;
	            _this.hero[0].hero.scaleX += .025;
	            _this.hero[0].hero.scaleY += .025;
	          }
