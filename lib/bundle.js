...
/***/ function(module, exports, __webpack_require__) {
	...
	document.addEventListener("DOMContentLoaded", function () {
	  ...
	  // for (let i in Cookies.get()) { Cookies.remove(i) }
	  ...
	  var title = new createjs.Text('Fishy', "100px Arial", "#e80000");
	  title.x = 260;
	  title.y = 110;
	  // stage.addChild(title);
	  stage.update();
	
	  (0, _jquery2.default)('#instructions').click(function () {
	    stage.removeAllChildren();
	    (0, _jquery2.default)('#play').removeClass('hidden');
	    var howToPlay = new createjs.Text('How to Play', "42px Arial", "#05B");
	    var movement = new createjs.Text("Use 'w', 'a', 's', 'd' to move", "28px Arial", "#201");
	    var directions = new createjs.Text("Eat the smaller fish to grow bigger while \navoiding getting eaten by the larger fish", "18px Arial", "#201");
	  ...
	  _createClass(Game, [{
	    ...
	    value: function inputHighscore() {
	      ...
	      var setHighscore = function setHighscore() {
	        ...
	        Cookies.set("" + Math.random(), { score: initials });
	        window.location.reload();
	      };
	      // debugger
	      $($('.submitHighscore')[1]).click(function (e) {
	        setHighscore.bind(_this)();
	      });
	    }
	  }, {
	    key: "detectCollision",
	      ...
	      function handleTick(e) {
	        ...
	        this.fish.forEach(function (el, idx) {
	          ...
	          if (Util.wrap(el, stage)) {
	            ...
	            newFish = [].concat(_this2.fish.splice(0, idx).concat(_this2.fish.splice(1)));
	            _this2.fish = newFish;
	          }
	          if (Util.collision(el, _this2.hero[0])) {
	            console.log('collision!');
	            if (el.fish.width > _this2.hero[0].hero.width) {
	              createjs.Ticker.reset();
	              var lose = new createjs.Text("Game Over", "64px Arial", "red");
	              lose.x = 230;
	              lose.y = 190;
	              stage.addChild(lose);
	              stage.removeChild(_this2.hero[0].hero);
	              stage.update();
	            ...
	            if (_this2.hero[0].hero.width > stage.canvas.width) {
	              createjs.Ticker.reset();
	              ...
	              _this2.fish.forEach(function (el) {
	                stage.removeChild(el.fish);
	              });
	              stage.update();
	              $('#replay').removeClass('hidden');
	              _this2.gameOver = true;
	              _this2.inputHighscore.bind(_this2)();
	              return;
	            }
	            stage.removeChild(el.fish);
	            newFish = [].concat(_this2.fish.splice(0, idx).concat(_this2.fish.splice(1)));
	            _this2.fish = newFish;
	            _this2.hero[0].hero.scaleX += .25;
	            _this2.hero[0].hero.scaleY += .25;
	          }
...
/***/ function(module, exports) {
	...
	var Hero = function () {
	  ...
	  _createClass(Hero, [{
	    ...
	    key: 'move',
	    value: function move() {
	      var nextX = this.pos[0] + 1;
	      var nextY = this.pos[1] + 1;
