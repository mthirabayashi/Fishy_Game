...
/***/ function(module, exports, __webpack_require__) {
	...
	document.addEventListener("DOMContentLoaded", function () {
	  (0, _jquery2.default)('.submitHighscore').addClass('hidden');
	  var allScores = Cookies.getJSON();
	  var highscores = [];
	  ...
	  for (var i = 0; i < highscores.length; i++) {
	    var _key = Object.keys(highscores[i]);
	    var score = Util.timeString(_key);
	    score += " - " + highscores[i][_key];
	    (0, _jquery2.default)("#highscores li:nth-child(" + (i + 1) + ")").text(score);
	  }
	
	  (0, _jquery2.default)('#reset-times').click(function (e) {
	    for (var _i in Cookies.get()) {
	      Cookies.remove(_i);
	  ...
	  (0, _jquery2.default)('#replay').addClass('hidden');
	  (0, _jquery2.default)('#speaker-icon').addClass('hidden');
	  (0, _jquery2.default)('#mute-icon').addClass('hidden');
	
	  (0, _jquery2.default)('#replay').click(function () {
	    window.location.reload();
	  });
	  ...
	  (0, _jquery2.default)('#play').click(function () {
	    ...
	    (0, _jquery2.default)('#instructions').addClass('hidden');
	    var game = new Game(stage);
	  });
	  // disable arrow key movement on window
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Fish = __webpack_require__(5);
	var Util = __webpack_require__(6);
	...
	var Game = function () {
	  function Game(stage) {
	    ...
	    // this would allow for a single high score rather than fastest winning times.  Could also c
	    this.score = 0;
	    this.gameOver = false;
	    this.mute = false;
	  ...
	  _createClass(Game, [{
	    ...
	    key: "createHero",
	    value: function createHero(stage) {
	      var hero = new Hero(stage);
	      this.hero = [hero];
	    }
	  }, {
	    key: "createFish",
	    value: function createFish(stage) {
	      for (var i = 0; i < 5; i++) {
	        var fish = new Fish(stage);
	    ...
	    value: function inputHighscore() {
	      ...
	      var setHighscore = function setHighscore() {
	        ...
	        var scoreObject = {};
	        scoreObject[_this2.score] = initials;
	        Cookies.set("" + Math.random(), scoreObject);
	        window.location.reload();
	      };
	      $($('.submitHighscore')[1]).one('click', function (e) {
	        setHighscore.bind(_this2)();
	      });
	    }
	  }, {
	    key: "detectCollision",
	    value: function detectCollision(stage) {
	      ...
	      function handleTick(e) {
	        ...
	        var newFish = void 0;
	        this.fish.forEach(function (el, idx) {
	          ...
	          if (Util.wrap(el, stage)) {
	            ...
	            newFish = [].concat(_this3.fish.splice(0, idx).concat(_this3.fish.splice(1)));
	            _this3.fish = newFish;
	          }
	          if (Util.collision(el, _this3.hero[0])) {
	            // console.log('collision!');
	            if (el.fish.width > _this3.hero[0].hero.width) {
	              _this3.loseGame.bind(_this3)(stage);
	              return;
	            }
	            if (_this3.hero[0].hero.width > stage.canvas.width * .7) {
	              _this3.winGame.bind(_this3)(stage);
	              return;
	            }
	            stage.removeChild(el.fish);
	            newFish = [].concat(_this3.fish.splice(0, idx).concat(_this3.fish.splice(1)));
	            _this3.fish = newFish;
	            _this3.hero[0].hero.scaleX += .03;
	            _this3.hero[0].hero.scaleY += .03;
	            if (_this3.mute === false) {
	              _this3.sound.play();
	            }
	          }
	    ...
	    key: "generateFish",
	    value: function generateFish(stage) {
	    ...
	    key: "loseGame",
	    value: function loseGame(stage) {
	      createjs.Ticker.reset();
	      var lose = new createjs.Text("Game Over", "64px Arial", "red");
	      lose.x = 230;
	      lose.y = 190;
	      stage.addChild(lose);
	    ...
	    key: "winGame",
	    value: function winGame(stage) {
	      createjs.Ticker.reset();
	      var win = new createjs.Text("You Won!", "64px Arial", "#05B");
	      win.x = 260;
	      win.y = 190;
	      stage.addChild(win);
	      // stage.removeChild(this.hero[0].hero);
	      this.fish.forEach(function (el) {
	        stage.removeChild(el.fish);
	      });
	      stage.update();
	      $('#replay').removeClass('hidden');
	...
	var Hero = function () {
	  ...
	  _createClass(Hero, [{
	    ...
	    value: function draw(stage) {
	      ...
	      var hero = new createjs.Bitmap("./images/hero_transparent.png");
	      hero.scaleX = 0.2;
	      hero.scaleY = 0.2;
	      ...
	      key('left, a', function () {
	        if (_this.heroX > -8) {
	          ...
	          if (_this.hero.rotation > 0) {
	            ...
	            _this.hero.skewX = 0;
	            _this.hero.x -= _this.hero.image.width * _this.hero.scaleX;
	          }
	        }
	      });
	      key('right, d', function () {
	        if (_this.heroX < 8) {
	          _this.heroX += 2.0;
	          if (_this.hero.rotation === 0) {
	            _this.hero.rotation = 180;
	            _this.hero.skewX = 180;
	            _this.hero.x += _this.hero.image.width * _this.hero.scaleX;
	          }
	        }
	      });
	      ...
	      function handleTick(e) {
	        ...
	        var heroCenterX = hero.x + hero.width / 2;
	        var heroCenterY = hero.y + hero.height / 2;
...
/***/ function(module, exports) {
	...
	var Fish = function () {
	  ...
	  _createClass(Fish, [{
	    ...
	    key: "randomPosition",
	    value: function randomPosition(stage) {
	      var x = Math.floor(Math.random() + .5);
	      // console.log(x);
	      if (x === 1) {
	        x = stage.canvas.width;
	      }
	      var y = Math.random() * (stage.canvas.height - 50);
	      return [x, y];
	    }
	  }, {
	    key: "setVelocity",
	    value: function setVelocity(fish) {
	      // console.log(fish);
	      if (fish.x === 0) {
	    ...
	    key: "draw",
	    value: function draw(stage) {
	      var fish = new createjs.Bitmap("./images/transparent.png");
	      fish.scaleX = this.size;
	      fish.scaleY = this.size;
	      fish.width = 100 * this.size;
	      fish.height = 70 * this.size;
	      var pos = this.randomPosition.bind(this)(stage);
	      fish.x = pos[0];
	      fish.y = pos[1];
...
/***/ function(module, exports) {
	...
	var Util = {
	  collision: function collision(el, hero) {
	    // console.log(hero);
	    var heroCenterX = void 0;
	    var fishCenterX = void 0;
	    var heroCenterY = hero.hero.y + hero.hero.height / 2.2;
	    var fishCenterY = el.fish.y + el.fish.height / 2.2;
	    var fishWidth = el.fish.width;
	    var fishHeight = el.fish.height;
	    var heroWidth = hero.hero.width;
	    var heroHeight = hero.hero.height;
	  ...
	  createFishBorder: function createFishBorder(el, stage) {
	    var fishWidth = el.fish.width;
	    var fishHeight = el.fish.height;
	    var fishBorder = new createjs.Shape();
	    if (el.fish.rotation === 0) {
	  ...
	  createHeroBorder: function createHeroBorder(hero, stage) {
	    var heroWidth = hero.hero.width;
	    var heroHeight = hero.hero.height;
	    var heroBorder = new createjs.Shape();
	    if (hero.hero.rotation === 0) {
